...
public class JwtProvider {
    ...
    public String generateToken(Authentication authentication) {
        ...
        List<String> roles = usuarioPrincipal.getAuthorities().stream().map(GrantedAuthority::getAuthority)
                .collect(Collectors.toList());// lista de cadena autorities en cadenas
    ...
    public String getNombreUsuarioFromToken(String token) {
        return Jwts.parser().setSigningKey(secret.getBytes()).parseClaimsJws(token).getBody().getSubject();// firmamos
                                                                                                           // con
                                                                                                           // secret, se
                                                                                                           // pasa el
                                                                                                           // token,
