package Plantilla.apirest.seguridad.jwt;
...
// Mas importante
// Se ejecuta por cada peticion, comprueba que sea valido el token usando provider,
// si es valido permite el acceso al recurso de lo contrario lanza la excepcion
    ...
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain filterChain)
        ...
        try {
            ...
            if (token != null && jwtProvider.validateToken(token)) { // si existe el token
                ...
                String nombreUsuario = jwtProvider.getNombreUsuarioFromToken(token);// con el token se obtiene el
                                                                                    // usuario
                UserDetails userDetails = userDetailsServiceImpl.loadUserByUsername(nombreUsuario);// Se crea un
                                                                                                   // userDetails y se
                                                                                                   // autentica una ves
                                                                                                   // se atiende ese
                                                                                                   // usuario
